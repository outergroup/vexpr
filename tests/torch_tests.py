    ...
    def test_sum_to_index_add(self):
        ...
        def expected(x1, x2):
            ...
            return vctorch.sum_multi(
                vctorch.split_and_stack(vtorch.stack([x1, x2, x1, x2]),
                                        **split_stack_kwargs),
                dim=0)
    ...
    def test_branching_stack(self):
        ...
        def expected(x1, x2):
            ...
            return vtorch.cat([
                matern(
                    vctorch.cdist_multi(
                        vctorch.split_and_stack(x1[..., indices12],
                                                **split_stack_kwargs),
                        vctorch.split_and_stack(x2[..., indices12],
                                                **split_stack_kwargs),
                        p=2)),
                vtorch.exp(
                    -vctorch.cdist_multi(
                        vctorch.split_and_stack(x1[..., indices34],
                                                **split_stack_kwargs),
                        vctorch.split_and_stack(x2[..., indices34],
                                                **split_stack_kwargs),
                        p=1))
            ])
